...
<%  //取出List
	List<RmLockVo> lResult = null;  //定义结果列表的List变量
	if(request.getAttribute(IRmLockConstants.REQUEST_BEANS) != null) {  //如果request中的beans不为空
		lResult = (List)request.getAttribute(IRmLockConstants.REQUEST_BEANS);  //赋值给resultList
	}
%>
...
<script type="text/javascript">
	...
	function toAdd_onClick() {  //到增加记录页面
		window.location="<%=request.getContextPath()%>/modules/lock/rmlock" + rmJspPath + "/insertRmLock.jsp";
	}
...
<div id="div_simple">
	<table class="table_query">
		...
		<tr>
			...
			<td>
				<input type="text" class="text_field" name="user_id" inputName="<%=IRmLockConstants.TABLE_COLUMN_CHINESE.get("user_id")%>" maxLength="50"/>
			</td>
			<td align="right"></td>
			<td></td>
		</tr>
		<tr>
			<td align="right"><%=IRmLockConstants.TABLE_COLUMN_CHINESE.get("bs_keyword")%></td>
			<td>
				<input type="text" class="text_field" name="bs_keyword" inputName="<%=IRmLockConstants.TABLE_COLUMN_CHINESE.get("bs_keyword")%>" maxLength="100"/>
			</td>
			<td align="right"></td>
			<td></td>
		</tr>
		<tr>
			<td align="right"><%=IRmLockConstants.TABLE_COLUMN_CHINESE.get("lock_content")%></td>
			<td>
				<input type="text" class="text_field" name="lock_content" inputName="<%=IRmLockConstants.TABLE_COLUMN_CHINESE.get("lock_content")%>" maxLength="100"/>
			</td>
			<td align="right"></td>
			<td></td>
		</tr>
		<tr>
			<td align="right"><%=IRmLockConstants.TABLE_COLUMN_CHINESE.get("lock_date")%></td>
			<td>
				<input type="text" class="text_field_half_reference_readonly" name="lock_date_from" inputName="<%=IRmLockConstants.TABLE_COLUMN_CHINESE.get("lock_date")%>"/><img class="refButtonClass" src="<%=request.getContextPath()%>/images/09.gif" onClick="javascript:getYearMonthDay(form.lock_date_from,'<%=request.getContextPath()%>/');"/>&nbsp;到&nbsp;<input type="text" class="text_field_half_reference_readonly" name="lock_date_to" inputName="<%=IRmLockConstants.TABLE_COLUMN_CHINESE.get("lock_date")%>"/><img class="refButtonClass" src="<%=request.getContextPath()%>/images/09.gif" onClick="javascript:getYearMonthDay(form.lock_date_to,'<%=request.getContextPath()%>/');"/>
...
<table class="tableHeader">
  <tr>
    ...
    <td class="tableHeaderMiddleTd"><b><%=IRmLockConstants.TABLE_NAME_CHINESE %>列表</b></td>
...
<layout:collection name="beans" id="rmBean" styleClass="listCss" width="100%" indexId="rmOrderNumber" align="center" sortAction="0">
	...
	<layout:collectionItem width="8%" title='<%=IRmLockConstants.TABLE_COLUMN_CHINESE.get("user_id")%>' property="user_id" sortable="true">
	...
	<layout:collectionItem width="8%" title='<%=IRmLockConstants.TABLE_COLUMN_CHINESE.get("bs_keyword")%>' property="bs_keyword" sortable="true">
		<bean:define id="rmValue" name="rmBean" property="bs_keyword"/>
		<%="<a class='aul' onClick='javascript:detail_onClick(getRowHiddenId())'>"%>
		<%=org.quickbundle.tools.helper.RmStringHelper.prt(rmValue)%>
		<%="</a>"%>
	</layout:collectionItem>
	<layout:collectionItem width="8%" title='<%=IRmLockConstants.TABLE_COLUMN_CHINESE.get("lock_content")%>' property="lock_content" sortable="true">
		<bean:define id="rmValue" name="rmBean" property="lock_content"/>
		<%="<a class='aul' onClick='javascript:detail_onClick(getRowHiddenId())'>"%>
		<%=org.quickbundle.tools.helper.RmStringHelper.prt(rmValue)%>
		<%="</a>"%>
	</layout:collectionItem>
	<layout:collectionItem width="8%" title='<%=IRmLockConstants.TABLE_COLUMN_CHINESE.get("lock_date")%>' property="lock_date" sortable="true">
		<bean:define id="rmValue" name="rmBean" property="lock_date"/>
...
<script type="text/javascript">
<%  //表单回写
	if(request.getAttribute(IRmLockConstants.REQUEST_WRITE_BACK_FORM_VALUES) != null) {  //如果request中取出的表单回写bean不为空
		out.print(RmVoHelper.writeBackMapToForm((java.util.Map)request.getAttribute(IRmLockConstants.REQUEST_WRITE_BACK_FORM_VALUES)));  //输出表单回写方法的脚本
	}
%>
</script>