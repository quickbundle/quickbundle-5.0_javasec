...
<input type="hidden" name="type_keyword" value="<%=RmStringHelper.prt(resultVo.getType_keyword())%>"><table align="center">
	<tr>
		<td><br>
			<%if(!isReadOnly) { %>
...
new Array ('子表RmCodeData','<%=request.getContextPath()%>/RmCodeDataConditionAction.do?cmd=queryAll&code_type_id=<%=resultVo.getId()%><%=isReadOnly ? "&" + org.quickbundle.project.IGlobalConstants.REQUEST_IS_READ_ONLY + "=1" : ""%>')
);
</script>
<br/><br/>
