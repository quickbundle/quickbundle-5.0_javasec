...
<%
	...
	File f = RmZipHelper.getExcelFromFile(zipFile);
		    ...
		    rtVo = RmExcelHandler.getListDataFromSheet(f, RmCodeDataVo.class.getName(), new RmExcelHandler.IValidateData() {
		        public String isValid(Object obj) {
		            String errorMsg = "";
		            RmCodeDataVo vo = (RmCodeDataVo)obj;

	if(vo.getCode_type_id().length() == 0 ) { errorMsg += "Code_type_id不能为空;"; }
		
	if(vo.getData_key().length() == 0 ) { errorMsg += "Data_key不能为空;"; }
	...
	if(vo.getEnable_status().length() == 0 ) { errorMsg += "Enable_status不能为空;"; }
		    ...
		    if(rtVo.getErrorMsg() == null) {
		        RmCodeDataVo[] diseaseVos = (RmCodeDataVo[])rtVo.getLData().toArray(new RmCodeDataVo[0]);
...
<script type="text/javascript">
  	function import_onClick(){  //保存修改后的单条数据
	    ...
	    form.action="importRmCodeData.jsp";
    	form.submit();
	}
</script>
</head>
...
<table class="table_div_content">
	...
	<tr>
		...
		<td>
			<a href="#"><u>批量导入<%=IRmCodeDataConstants.TABLE_NAME_CHINESE %>模板</u></a>
		</td>
		<td align="right">&nbsp;</td>
		<td>&nbsp;</td>
	</tr>
