...
<script type="text/javascript">
	...
	function toAdd_onClick() {  //到增加记录页面
		form.action="<%=request.getContextPath()%>/modules/code/rmcodedata" + rmJspPath + "/insertRmCodeData.jsp";
		form.submit();
	}
	function toReadOnly_onClick() {  //到只读
		form.action="<%=request.getContextPath()%>/" + rmActionName + ".do?cmd=queryAll&<%=IRmCodeDataConstants.REQUEST_IS_READ_ONLY + "=1"%>";
		form.submit();
	}
	...
	function buildQueryCondition() {  //构建简单查询
		...
		pushCondition(qca, "code_type_id", "=", "");
...
<div id="div_simple" style="display:block;">
...
<%=isReadOnly ? "<input type=\"hidden\" name=\"" + IRmCodeDataConstants.REQUEST_IS_READ_ONLY + "\" value=\"1\">" : ""%>
</form>
<script type="text/javascript">
