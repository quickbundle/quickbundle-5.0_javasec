...
TabPanel = function(config){
  ...
  //this.tabWidth = 80+4;//样式.tabpanel_mover li中width+4(样式width80，需要一起修改)
  this.tabWidth = 0;//this.tabWidth = 0 不会出现滚动条)
...
TabPanel.prototype = {
  ...
  moveToRight : function(){
    ...
    if(this.scrolled && this.scrollFinish)
    {
      ...
      //需要移动多少
      var num = (liWidth - cWidth - marginLeft + this.fixNum)*-1;alert();
      Fader.apply(this, new Array({
        element:this.tabpanel_mover,
        style:'marginLeft',
        num:num,
  ...
  update : function(){
    ...
    this.maxMove = (sum) - cWidth + this.fixNum;
  },
  ...
  show : function(position, notExecuteMoveSee){
	//获取当前选中的选项卡id和名称
	 if( this.tabs[position] != "undefined" && this.tabs[position]!=undefined){
		 stotalcodeName=this.tabs[position].title.text();
		 stotalcode=this.tabs[position].id;
	 }
	//alert(this.tabs[position].title.text());	  
	//alert(this.tabs[position].id);
	//如果没有选项卡则停止方法执行
  ...
  kill : function(position){
	//alert(this.tabs[this.getTabPosision(position)].title.width());
	  
	//if(position!='10066' && position!='10068' && position!='1001002131020001') { //工作流审核页面专用
	//减去当前关闭选项卡的宽度
    sum = sum - this.tabs[this.getTabPosision(position)].title.width()-20;
	//}
	var tabEntity = this;
    //得到下标
    position = this.getTabPosision(position);
