<%@ page contentType="image/jpeg; charset=UTF-8" session="false" import="java.awt.*,java.awt.image.*,java.util.*,javax.imageio.*" %><%@page import="org.quickbundle.project.login.IRmLoginConstants"%><%@page import="org.quickbundle.project.RmProjectHelper"%><%@page import="org.quickbundle.tools.helper.RmJspHelper"%><%! 
//生成随机颜色 
...
//将认证码存入session 
{
	...
	session.setAttribute("randomStr",sRand);
}
//设置图片的长宽 
...
for (int i=0;i<sRand.length();i++)  {
    ...
    //设置字体 
    g.setFont(new Font(fontTypes[random.nextInt(fontTypesLength)],Font.BOLD,19 + random.nextInt(3))); 
    //将此汉字画到图片上 
    g.drawString(sRand.substring(i, i+1),16*i+ 2 + random.nextInt(8),18); 
}  
