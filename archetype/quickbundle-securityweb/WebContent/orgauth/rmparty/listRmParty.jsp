...
<%  //取出List
	List<RmPartyVo> lResult = null;  //定义结果列表的List变量
	if(request.getAttribute(IRmPartyConstants.REQUEST_BEANS) != null) {  //如果request中的beans不为空
		lResult = (List)request.getAttribute(IRmPartyConstants.REQUEST_BEANS);  //赋值给resultList
	}
%>
...
<script type="text/javascript">
	...
	function toAdd_onClick() {  //到增加记录页面
		window.location="<%=request.getContextPath()%>/orgauth/rmparty" + rmJspPath + "/insertRmParty.jsp";
	}
...
<div id="div_simple">
	<table class="table_query">
		...
		<tr>
			...
			<td>
				<input type="text" class="text_field" name="old_party_id" inputName="<%=IRmPartyConstants.TABLE_COLUMN_CHINESE.get("old_party_id")%>" maxLength="25"/>
			</td>
			<td align="right"></td>
			<td></td>
		</tr>
		<tr>
			...
			<td>
				<input type="text" class="text_field" name="name" inputName="<%=IRmPartyConstants.TABLE_COLUMN_CHINESE.get("name")%>" maxLength="100"/>
			</td>
			<td align="right"></td>
			<td></td>
		</tr>
		<tr>
			...
			<td>
				<%=RmJspHelper.getSelectField("is_inherit", -1, RmGlobalReference.get(IRmPartyConstants.DICTIONARY_RM_YES_NOT), "", "inputName='" + IRmPartyConstants.TABLE_COLUMN_CHINESE.get("is_inherit") + "'", true) %>
			</td>
			<td align="right"></td>
			<td></td>
		</tr>
		<tr>
			...
			<td>
				<input type="text" class="text_field" name="email" inputName="<%=IRmPartyConstants.TABLE_COLUMN_CHINESE.get("email")%>" maxLength="100"/>
			</td>
			<td align="right"></td>
			<td></td>
		</tr>
		<tr>
			...
			<td>
				<input type="text" class="text_field" name="description" inputName="<%=IRmPartyConstants.TABLE_COLUMN_CHINESE.get("description")%>" maxLength="500"/>
...
<table class="tableHeader">
  <tr>
    ...
    <td class="tableHeaderMiddleTd"><b><%=IRmPartyConstants.TABLE_NAME_CHINESE %>列表</b></td>
...
<layout:collection name="beans" id="rmBean" styleClass="listCss" width="100%" indexId="rmOrderNumber" align="center" sortAction="0">
	...
	<layout:collectionItem width="8%" title='<%=IRmPartyConstants.TABLE_COLUMN_CHINESE.get("party_type_id")%>' property="party_type_id" sortable="true"/>
	<layout:collectionItem width="8%" title='<%=IRmPartyConstants.TABLE_COLUMN_CHINESE.get("old_party_id")%>' property="old_party_id" sortable="true"/>
	<layout:collectionItem width="8%" title='<%=IRmPartyConstants.TABLE_COLUMN_CHINESE.get("name")%>' property="name" sortable="true">
	...
	<layout:collectionItem width="8%" title='<%=IRmPartyConstants.TABLE_COLUMN_CHINESE.get("is_inherit")%>' property="is_inherit" sortable="true">
		<bean:define id="rmValue" name="rmBean" property="is_inherit"/>
		<%=RmGlobalReference.get(IRmPartyConstants.DICTIONARY_RM_YES_NOT, rmValue)%>
	</layout:collectionItem>
	<layout:collectionItem width="8%" title='<%=IRmPartyConstants.TABLE_COLUMN_CHINESE.get("email")%>' property="email" sortable="true"/>
	<layout:collectionItem width="8%" title='<%=IRmPartyConstants.TABLE_COLUMN_CHINESE.get("description")%>' property="description" sortable="true"/>
	</layout:collection>
...
<div id="div_funcNode" style="padding:20px 10px 10px 0px; display:none" align="right">
	<a class="aul" target="_blank" href="<%=request.getContextPath()%>/RmPartyConditionAction.do?cmd=queryAll&<%=IRmPartyConstants.DEFAULT_CONDITION_KEY_ARRAY[0]%>=1">带条件模式</a>
	<a class="aul" target="_blank" href="<%=request.getContextPath()%>/RmPartyConditionAction.do?cmd=queryAll&<%=IRmPartyConstants.DEFAULT_CONDITION_KEY_ARRAY[0]%>=1&<%=IRmPartyConstants.REQUEST_IS_READ_ONLY%>=1">带条件只读</a>
	</div>
<!-- end -->
...
<script type="text/javascript">
<%  //表单回写
	if(request.getAttribute(IRmPartyConstants.REQUEST_WRITE_BACK_FORM_VALUES) != null) {  //如果request中取出的表单回写bean不为空
		out.print(RmVoHelper.writeBackMapToForm((java.util.Map)request.getAttribute(IRmPartyConstants.REQUEST_WRITE_BACK_FORM_VALUES)));  //输出表单回写方法的脚本
	}
%>
</script>