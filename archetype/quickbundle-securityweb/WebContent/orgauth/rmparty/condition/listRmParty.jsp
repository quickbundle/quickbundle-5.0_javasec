...
<%  //判断是否只读
	...
	if("1".equals(request.getAttribute(IRmPartyConstants.REQUEST_IS_READ_ONLY))) {
		isReadOnly = true;
	}
%>
<%  //取出List
...
<script type="text/javascript">
	var rmActionName = "RmPartyConditionAction";
	...
	function toAdd_onClick() {  //到增加记录页面
		form.action="<%=request.getContextPath()%>/orgauth/rmparty" + rmJspPath + "/insertRmParty.jsp";
		form.submit();
	}
...
<%=isReadOnly ? "<input type=\"hidden\" name=\"" + IRmPartyConstants.REQUEST_IS_READ_ONLY + "\" value=\"1\">" : ""%>
...
%>
autoPatchParentIdName("<%=IRmPartyConstants.DEFAULT_CONDITION_KEY_ARRAY[0]%>");
</script>