...
<%  //取出List
	List<RmPartyTypeRelationRuleVo> lResult = null;  //定义结果列表的List变量
	if(request.getAttribute(IRmPartyTypeRelationRuleConstants.REQUEST_BEANS) != null) {  //如果request中的beans不为空
		lResult = (List)request.getAttribute(IRmPartyTypeRelationRuleConstants.REQUEST_BEANS);  //赋值给resultList
	}
%>
...
<script type="text/javascript">
	var rmActionName = "RmPartyTypeRelationRuleAction";
	var rmJspPath = "";
	...
	function toAdd_onClick() {  //到增加记录页面
		window.location="<%=request.getContextPath()%>/orgauth/rmpartytyperelationrule" + rmJspPath + "/insertRmPartyTypeRelationRule.jsp";
	}
...
<div id="div_simple">
	<table class="table_query">
		...
		<tr>
			...
			<td>
				<input type="text" class="text_field" name="rule_desc" inputName="<%=IRmPartyTypeRelationRuleConstants.TABLE_COLUMN_CHINESE.get("rule_desc")%>" maxLength="2000"/>
			</td>
			<td align="right"></td>
			<td></td>
		</tr>
		<tr>
			...
			<td>
				<%=RmJspHelper.getSelectField("is_insert_child_party", -1, RmGlobalReference.get(IRmPartyTypeRelationRuleConstants.DICTIONARY_RM_YES_NOT), "", "inputName='" + IRmPartyTypeRelationRuleConstants.TABLE_COLUMN_CHINESE.get("is_insert_child_party") + "'", true) %>
...
<table class="tableHeader">
  <tr>
    ...
    <td class="tableHeaderMiddleTd"><b><%=IRmPartyTypeRelationRuleConstants.TABLE_NAME_CHINESE %>列表</b></td>
...
<layout:collection name="beans" id="rmBean" styleClass="listCss" width="100%" indexId="rmOrderNumber" align="center" sortAction="0">
	...
	<%
		...
		RmPageVo pageVo = (RmPageVo)pageContext.getRequest().getAttribute(IRmPartyTypeRelationRuleConstants.RM_PAGE_VO);
		out.print((pageVo.getCurrentPage() - 1) * pageVo.getPageSize() + rmOrderNumber.intValue() + 1);
	%>
		<bean:define id="rmValue" name="rmBean" property="id"/>
		<input type="hidden" signName="hiddenId" value="<%=rmValue%>"/>
	</layout:collectionItem>
	<layout:collectionItem width="8%" title='<%=IRmPartyTypeRelationRuleConstants.TABLE_COLUMN_CHINESE.get("party_view_id")%>' property="party_view_id" sortable="true">
		<bean:define id="rmValue" name="rmBean" property="party_view_id"/>
		<%="<a class='aul' onClick='javascript:detail_onClick(getRowHiddenId())'>"%>
		<%=org.quickbundle.tools.helper.RmStringHelper.prt(rmValue)%>
		<%="</a>"%>
	</layout:collectionItem>
	<layout:collectionItem width="8%" title='<%=IRmPartyTypeRelationRuleConstants.TABLE_COLUMN_CHINESE.get("parent_party_type_id")%>' property="parent_party_type_id" sortable="true">
		<bean:define id="rmValue" name="rmBean" property="parent_party_type_id"/>
		<%="<a class='aul' onClick='javascript:detail_onClick(getRowHiddenId())'>"%>
		<%=org.quickbundle.tools.helper.RmStringHelper.prt(rmValue)%>
		<%="</a>"%>
	</layout:collectionItem>
	<layout:collectionItem width="8%" title='<%=IRmPartyTypeRelationRuleConstants.TABLE_COLUMN_CHINESE.get("child_party_type_id")%>' property="child_party_type_id" sortable="true">
		<bean:define id="rmValue" name="rmBean" property="child_party_type_id"/>
		<%="<a class='aul' onClick='javascript:detail_onClick(getRowHiddenId())'>"%>
		<%=org.quickbundle.tools.helper.RmStringHelper.prt(rmValue)%>
		<%="</a>"%>
	</layout:collectionItem>
	<layout:collectionItem width="8%" title='<%=IRmPartyTypeRelationRuleConstants.TABLE_COLUMN_CHINESE.get("rule_desc")%>' property="rule_desc" sortable="true">
		<bean:define id="rmValue" name="rmBean" property="rule_desc"/>
		<%="<a class='aul' onClick='javascript:detail_onClick(getRowHiddenId())'>"%>
		<%=org.quickbundle.tools.helper.RmStringHelper.prt(rmValue)%>
		<%="</a>"%>
	</layout:collectionItem>
	<layout:collectionItem width="8%" title='<%=IRmPartyTypeRelationRuleConstants.TABLE_COLUMN_CHINESE.get("is_insert_child_party")%>' property="is_insert_child_party" sortable="true">
		<bean:define id="rmValue" name="rmBean" property="is_insert_child_party"/>
		<%=RmGlobalReference.get(IRmPartyTypeRelationRuleConstants.DICTIONARY_RM_YES_NOT, rmValue)%>
	</layout:collectionItem>
	</layout:collection>
...
<div id="div_funcNode" style="padding:20px 10px 10px 0px; display:none" align="right">
	<a class="aul" target="_blank" href="<%=request.getContextPath()%>/RmPartyTypeRelationRuleConditionAction.do?cmd=queryAll&<%=IRmPartyTypeRelationRuleConstants.DEFAULT_CONDITION_KEY_ARRAY[0]%>=1">带条件模式</a>
	<a class="aul" target="_blank" href="<%=request.getContextPath()%>/RmPartyTypeRelationRuleConditionAction.do?cmd=queryAll&<%=IRmPartyTypeRelationRuleConstants.DEFAULT_CONDITION_KEY_ARRAY[0]%>=1&<%=IRmPartyTypeRelationRuleConstants.REQUEST_IS_READ_ONLY%>=1">带条件只读</a>
	</div>
<!-- end -->
...
<script type="text/javascript">
<%  //表单回写
	if(request.getAttribute(IRmPartyTypeRelationRuleConstants.REQUEST_WRITE_BACK_FORM_VALUES) != null) {  //如果request中取出的表单回写bean不为空
		out.print(RmVoHelper.writeBackMapToForm((java.util.Map)request.getAttribute(IRmPartyTypeRelationRuleConstants.REQUEST_WRITE_BACK_FORM_VALUES)));  //输出表单回写方法的脚本
	}
%>
</script>