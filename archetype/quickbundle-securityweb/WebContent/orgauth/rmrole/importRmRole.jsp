...
<%
	...
	try {
		if(isSubmit) {
		    ...
		    rtVo = RmExcelHandler.getListDataFromSheet(f, RmRoleVo.class.getName(), new RmExcelHandler.IValidateData() {
		        public String isValid(Object obj) {
		            String errorMsg = "";
		            RmRoleVo vo = (RmRoleVo)obj;

	if(vo.getRole_code().length() == 0 ) { errorMsg += "Role_code不能为空;"; }
				
	if(vo.getName().length() == 0 ) { errorMsg += "Name不能为空;"; }
				
	if(vo.getEnable_status().length() == 0 ) { errorMsg += "Enable_status不能为空;"; }
				
	if(vo.getIs_system_level().length() == 0 ) { errorMsg += "Is_system_level不能为空;"; }
		    ...
		    if(rtVo.getErrorMsg() == null) {
		        RmRoleVo[] diseaseVos = (RmRoleVo[])rtVo.getLData().toArray(new RmRoleVo[0]);
...
<table class="table_div_content">
	...
	<tr>
		...
		<td>
			<a href="#"><u>批量导入<%=IRmRoleConstants.TABLE_NAME_CHINESE %>模板</u></a>
		</td>
		<td align="right">&nbsp;</td>
		<td>&nbsp;</td>
	</tr>
