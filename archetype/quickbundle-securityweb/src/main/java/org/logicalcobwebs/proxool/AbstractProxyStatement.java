...
abstract class AbstractProxyStatement {
	//qb-rm
	private static final Logger logSql = RmLogHelper.getLogger("rmsql");
    ...
    //qb-rm
    protected StringBuffer getSqlLog() {
		return sqlLog;
	}
    ...
    protected void trace(long startTime, Exception exception) throws SQLException {
        ...
        if (isTrace()) {
            ...
            sb.append(".");
            sb.append(this.proxyConnection.getId());
            sb.append(".");
            sb.append(RmGlobalMonitor.getShortTransactionUUID(String.valueOf(this.proxyConnection.getId())));
            sb.append("|s|");
            if(!this.sqlLog.toString().equals(this.sqlStatement)) {
            ...
            logSql.debug(sb.toString().replaceAll("[\\r\\n]+", " ").trim());
            //记录sql执行时间
            long[] sqlCount = RmRequestMonitor.tlSqlCount.get();
            if(sqlCount != null) {
            	sqlCount[3] += executeTime;
            }
